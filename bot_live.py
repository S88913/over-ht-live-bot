...
def match_names(name, candidates):
    return difflib.get_close_matches(name.lower(), [c.lower() for c in candidates], n=1, cutoff=0.7)
...
def main():
    ...
    try:
        with open(MATCH_FILE, newline='', encoding='utf-8') as f:
            ...
            for row in reader:
                try:
                    perc_str = row["Over05 FHG HT Average"]
                    if not perc_str.strip().replace('.', '', 1).isdigit():
                        continue
                    perc = float(perc_str)
                    if perc < PERCENTAGE_THRESHOLD:
                        continue

                    home_csv = row["Home Team"].strip()
                    away_csv = row["Away Team"].strip()

                    for ev in events:
                        home_live = ev.get("home", {}).get("name", "").strip()
                        away_live = ev.get("away", {}).get("name", "").strip()
                        matched_home = match_names(home_csv, [home_live])
                        matched_away = match_names(away_csv, [away_live])
