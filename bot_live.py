...
def get_live_events():
    ...
    try:
        ...
        print("❌ Errore caricamento eventi live:", e)
        return []
...
def main():
    print("🚀 Bot LIVE avviato – monitoraggio attivo ogni 30 secondi")
    notified = load_notified_ids()

    while True:
        print(f"🔄 Controllo alle {datetime.now(pytz.timezone('Europe/Rome')).strftime('%H:%M:%S')}...")
                        ...
                        for ev in events:
                            ...
                            if home in home_live and away in away_live:
                                ...
                                if quota is not None and quota >= 2.00:
                                    ...
                                    print("📌 Notifica inviata per:", match_id)
                                break
                    except Exception as e:
