        ...
        with open(MATCH_FILE, newline='', encoding='utf-8') as f:
            ...
            for row in reader:
                try:
                    ...
                    home_csv = normalize(row["Home Team"])
                    away_csv = normalize(row["Away Team"])

                    for ev in events:
                        home_live = normalize(ev.get("home", {}).get("name", ""))
                        away_live = normalize(ev.get("away", {}).get("name", ""))
